...
class DifferentialExpressionAnalysisEndpoint(APIView):
    ...
    def post(self, request):
        print("From api.views.diff_gene_analysis. activate differential_expression_analysis")
        try:
            # 1. 確保 body 有內容
            if not request.body:
            ...
            if not species:
                return Response({'error': 'Missing required field: species'}, status=status.HTTP_400_BAD_REQUEST)
            if method not in {'edgeR', 'DESeq2'}:
                return Response({'error': 'Invalid method. Use edgeR or DESeq2.'}, status=status.HTTP_400_BAD_REQUEST)
            if not isinstance(meta_data, list) or not meta_data:
                return Response({'error': 'all_columns_map must be a non-empty list'}, status=status.HTTP_400_BAD_REQUEST)
            if method == 'edgeR':
            ...
            if os.path.exists(count_file_tsv):
                ...
                return Response({'error': f'Count file not found for species: {species}'}, status=status.HTTP_404_NOT_FOUND)
